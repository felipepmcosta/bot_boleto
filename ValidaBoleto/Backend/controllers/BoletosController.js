  ...
  async consultaBoleto(req, res) {
    ...
    try {
      ...
      const resposta = await BoletosGeral.findAll({
        ...
        order: [['geracao', 'DESC']], // Ordena por data_registro em ordem decrescente
      ...
      if ((resposta.length > 0)) {
        ...
        if (!(token === null)) {
          ...
          return res.status(200).json({ resultado: false, varRet: "Sr.Responsável, não foi possível o processamento da Solicitação. O PDF não foi gerado para este aluno pela unidade. Favor entrar em contato com a Secretaria." });
        }
      }
      else {
